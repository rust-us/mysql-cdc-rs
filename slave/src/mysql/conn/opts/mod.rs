...
use native_tls_opts::ClientIdentity;
...
impl Opts {
    ...
    /// name            | value
    /// ----------------|--------------------------
    /// _client_name    | The client library name
    /// _client_version | The client library version
    /// _os             | The operation system
    /// _pid            | The client process ID
    /// _platform       | The machine platform
    ...
    ///
    /// ```
    /// # use slave::mysql::conn::opts::Opts;
    /// # fn main() -> Result<()> {
...
mod test {
    ...
    //     crate::mysql::conn::connection::new(url).unwrap();
    //     crate::mysql::conn::connection::new(opts.clone()).unwrap();
    //     crate::mysql::conn::connection::new(opts_builder.clone()).unwrap();
    ...
    fn should_have_url_err() {
        ...
        assert_eq!(
            parsed,
            Err(error::UrlError::InvalidValue(
                "port".to_string(),
                "NOTAPORT".to_string()
            ))
        );
