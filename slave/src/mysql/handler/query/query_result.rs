...
pub trait Protocol: 'static + Send + Sync {
    fn next(conn: &mut Connection, columns: Arc<[Column]>) -> Result<Option<Row>>;
}
...
#[derive(Debug)]
pub struct QueryResult<'c, T: Protocol> {
    conn: ConnMut<'c>,
...
impl<'c, T: Protocol> QueryResult<'c, T> {
    fn from_state(conn: ConnMut<'c>, state: SetIteratorState) -> QueryResult<'c, T> {
        QueryResult {
            conn,
            state,
            set_index: 0,
    ...
    pub fn next_set<'d>(&'d mut self) -> Option<ResultSet<'c, 'd, T>> {
        self.iter()
    }
    ...
    /// ```rust
    /// ```
    pub fn iter<'d>(&'d mut self) -> Option<ResultSet<'c, 'd, T>> {
...
impl<'c, T: Protocol> Drop for QueryResult<'c, T> {
    fn drop(&mut self) {
...
impl<'a, T: Protocol> std::ops::Deref for ResultSet<'a, '_, T> {
...
impl<T: Protocol> Iterator for ResultSet<'_, '_, T> {
...
impl<T: Protocol> Iterator for QueryResult<'_, T> {
    type Item = Result<Row>;
...
impl<T: Protocol> Drop for ResultSet<'_, '_, T> {
    fn drop(&mut self) {
