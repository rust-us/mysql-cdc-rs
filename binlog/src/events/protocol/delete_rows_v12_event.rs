...
impl DeleteRowsEvent {
    ...
    pub fn new(header: Header,
               ...
               columns_number: usize, deleted_image_bits: Vec<bool>,
    ...
    pub fn parse(
        cursor: &mut Cursor<&[u8]>, slice: &[u8],
        table_map: &HashMap<u64, TableMapEvent>,
        header: &Header, context: Rc<RefCell<LogContext>>) -> Result<DeleteRowsEvent, ReError> {
        ...
        let rows = parse_row_data_list(&mut _rows_data_cursor, table_map, table_id, &deleted_image_bits)?;

        let checksum = cursor.read_u32::<LittleEndian>()?;

        Ok(DeleteRowsEvent {
            header: Header::copy_and_get(header, checksum, vec![]),
            table_id,
            flags,
            extra_data_len,
            extra_data,
