...
use binlog::utils::{read_null_term_string_with_cursor, read_string};
...
impl HandshakePacket {
    pub fn parse(packet: &[u8]) -> CResult<Self> {
        ...
        let mut scramble = read_null_term_string_with_cursor(&mut cursor)?;
        ...
        if (server_capabilities & capability_flags::CLIENT_SECURE_CONNECTION) > 0 {
            scramble += &read_null_term_string_with_cursor(&mut cursor)?;
        }

        let mut auth_plugin_name = String::new();
        if (server_capabilities & capability_flags::CLIENT_PLUGIN_AUTH) > 0 {
            auth_plugin_name = read_null_term_string_with_cursor(&mut cursor)?;
        }
